...
while True:
    ...
    if command == 1:
        ...
        print(f'У студента по имени {student} по {class_с} следующие оценки  {students_marks[student][class_с]}')
        mark_del = int(input('Введите оценку для редактирования: '))
        mark_new = int(input('Введите оценку на которую нужно изменить: '))
        if student in students_marks.keys() and class_с in students_marks[student].keys():
            nn = students_marks[student][class_с]
            index_mark = nn.index(mark_del)
            nn.remove(mark_del)
            nn.insert(index_mark, mark_new)
            print(f''' Для студента по имени {student} по {class_с} изменена оценка {mark_del}
            Текущие оценки у студента {student} = {students_marks[student][class_с]} ''')
            del nn
        else:
        ...
        print('8. Вывод информации по всем оценкам по определенному ученику')
        student = input('Введите имя ученика: ')
        if student in students_marks.keys():
            for classes, marks in students_marks[student].items():
                print(f'{classes} - {students_marks[student][classes]}')
            print()
    elif command == 10:
